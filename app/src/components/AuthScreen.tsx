...
export function AuthScreen({ onAuthenticated }: AuthScreenProps) {
  ...
  const deptLookupTimer = useRef<ReturnType<typeof setTimeout> | null>(null)
  ...
  function handleEmailChange(value: string) {
    ...
    if (!value.trim()) {
      ...
      setRegistrationDept(null)
    } else {
      setEmailMenuOpen(false)
    }
    if (mode === 'register') {
      if (deptLookupTimer.current) clearTimeout(deptLookupTimer.current)
      const trimmed = value.trim()
      if (!trimmed.includes('@')) {
        setRegistrationDept(null)
        return
      }
      deptLookupTimer.current = setTimeout(() => {
        void window.spravochnik
          .getRegistrationDepartment(trimmed)
          .then((info) => setRegistrationDept(info))
          .catch(() => setRegistrationDept(null))
      }, 350)
    }
  }
  ...
  function pickSavedLogin(item: RememberedLogin) {
    ...
    if (mode === 'register') {
      void window.spravochnik
        .getRegistrationDepartment(item.email)
        .then(setRegistrationDept)
        .catch(() => setRegistrationDept(null))
    }
  }
          ...
          <button
            ...
            onClick={() => {
              ...
              if (email.trim().includes('@')) {
                void window.spravochnik
                  .getRegistrationDepartment(email.trim())
                  .then(setRegistrationDept)
                  .catch(() => setRegistrationDept(null))
              }
            }}
        ...
        <form className="auth-form" onSubmit={handleSubmit}>
          ...
          {mode === 'register' && (
            <label className="field">
              ...
              <input
                type="text"
                value={registrationDept?.label ?? ''}
                readOnly
                disabled
                placeholder="Укажите почту из белого списка"
              />
            </label>
          )}
