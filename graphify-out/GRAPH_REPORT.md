...
- `PublicUser` --references--> `UserRole`  [EXTRACTED]
  app/electron/auth-store.ts → app/electron/paths.ts
- `mergeGuidesWithRemote()` --calls--> `mergeGuideFile()`  [EXTRACTED]
  app/electron/yandex-sync.ts → app/electron/guide-merge.ts
- `resolveSyncConflicts()` --calls--> `applyConflictResolutions()`  [EXTRACTED]
  app/electron/yandex-sync.ts → app/electron/guide-merge.ts
...
## Knowledge Gaps
- **142 isolated node(s):** `SettingsData`, `SessionData`, `ROLE_RANK`, `GuideListKey`, `GuideTopic` (+137 more)
