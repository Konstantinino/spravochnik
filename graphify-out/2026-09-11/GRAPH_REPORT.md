...
- `HeaderProps` --references--> `DepartmentId`  [EXTRACTED]
  app/src/components/Header.tsx → app/src/types.ts
- `SettingsPageProps` --references--> `PublicUser`  [EXTRACTED]
  app/src/components/SettingsPage.tsx → app/src/types.ts
- `SpravochnikApi` --references--> `ConflictResolution`  [EXTRACTED]
  app/src/vite-env.d.ts → app/src/types.ts
...
## Knowledge Gaps
- **378 isolated node(s):** `Текущая фаза`, `Сервер (`server/`)`, `Клиент (`app/`)`, `Скрипты и восстановление`, `Данные` (+373 more)
...
- **Why does `SettingsPage()` connect `SettingsPage` to `types.ts`, `App.tsx`?**
  _High betweenness centrality (0.012) - this node is a cross-community bridge._
...
- **What connects `Текущая фаза`, `Сервер (`server/`)`, `Клиент (`app/`)` to the rest of the system?**
