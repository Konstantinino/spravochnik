...
import { clearSessionCookie, setSessionCookie } from '../lib/web-session.js'
...
async function credentialsLogin(
  ...
  | { ok: true; user: ReturnType<typeof toPublicUser>; token: string }
...
authRouter.post('/login', async (req, res) => {
  try {
    const result = await credentialsLogin(String(req.body.email ?? ''), String(req.body.password ?? ''))
    if (!result.ok) {
...
/** Browser SPA: JWT in httpOnly cookie, no token in response body. */
authRouter.post('/login-web', async (req, res) => {
  try {
    const result = await credentialsLogin(String(req.body.email ?? ''), String(req.body.password ?? ''))
    if (!result.ok) {
      res.status(result.status).json({ error: result.error })
...
authRouter.post('/logout-web', (_req, res) => {
...
/** Browser SPA registration — session cookie instead of token in JSON. */
authRouter.post('/register-web', async (req, res) => {
  try {
    const name = String(req.body.name ?? '').trim()
    const email = normalizeEmail(String(req.body.email ?? ''))
    const password = String(req.body.password ?? '')
