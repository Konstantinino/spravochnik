...
Локально на Windows (21 сент.): медиа `media/{отдел}/{id}/images|files`, фото на сервер при сохранении темы, автоподгрузка чужих правок без перезапуска, **тема остаётся открытой после сохранения** (Esc закрывает / «← Назад» при переходе по ссылке; ручная смена фильтра списка — сбрасывает выбор). Setup **1.4.4** (ПКМ «редактировать тему», combobox родителя, UX настроек/окна, сохранение ярлыка при update). Production: nginx `client_max_body_size` ≥120M для upload Setup (~81 МБ), иначе **413**. **Миграции 004–007** на production — reorder, категории «Ошибки»/«Дополнительно», блокировка порядка (`topic_order_locks`).
...
- [x] Пикеры родителя и ссылки «+»: поиск **только по названию** темы; родитель — темы **текущей категории** (party); «+» — все темы отдела (кроме архива)
- [x] **ParentTopicField:** combobox под чекбоксом «Сделать подтемой» (всегда виден; неактивен → «не выбрано»); смена категории сбрасывает чекбокс
- [x] **TopicList ПКМ:** «Редактировать порядок» / «Редактировать тему» (только по строке темы); reorder не открывает редактор
...
- [x] Клиент **1.4.4** (`REST-INFO-Setup-1.4.4.exe`): auto-update через `electron-updater` (фоновое скачивание, «Обновить» / «Скачивание…» в профиле, версия в шапке вплотную к «REST INFO»)
...
| Залить Setup **1.4.4** + `latest.yml` + blockmap на production | **Высокий** | Админ / программист |
